# FJ Analytics — Manual do Usuário
...
9. [Live Preview (Desenvolvimento)](#9-live-preview-desenvolvimento)
...
O **FJ Analytics** é um painel de inteligência de negócios (Business Intelligence) desenvolvido especificamente para a gestão da clínica veterinária. Ele centraliza dados de produção médica, comissões, custos operacionais e análises clínicas em um único ambiente, atualizado em tempo real via Firebase.
...
1. Abra o arquivo `index.html` no navegador ou acesse o link fornecido pelo administrador
...
> - O sistema registra data, hora e dispositivo de cada acesso
...
|--------|-----------|--------|
| 🏥 **LaborBI** | Gestão de comissões, produção veterinária e análises por setor clínico | Visão Geral · Ranking · Individual · Clínica · Internação · Cirúrgico · Lab |
| 📊 **CogsBI** | Gestão de custos, lucro bruto e inteligência financeira por DRE | DRE · EBITDA · Histórico · Categorias de custo |
...
Aba principal do módulo. Exibe a produção e comissão de cada veterinário no período selecionado, com filtros por veterinário, mês e semana.
...
### 4.3 Especialistas — Individual

Drill-down por veterinário: exibe a evolução mensal de produção, comissão acumulada, eficiência e setor de maior contribuição para o profissional selecionado.
...
>
> Ao analisar o LAB, observe **quem está solicitando mais**, não apenas o total geral.
...
### 5.1 Abas de Navegação

| Aba | O que exibe |
|-----|------------|
| **DRE** | Demonstrativo de Resultado — faturamento, custos por grupo, pré-tributação e lucro líquido |
| **EBITDA** | Resultado operacional em 6 camadas: Receita → Labor → COGS → Pré-trib. → Impostos → Lucro Líquido |
| **Histórico** | Evolução mês a mês de receita, custos agrupados e margens |
| **Categorias** | Uma aba por grupo de custo presente nos dados (Labor, Laboratório Ext., Insumos, Facilities, T.I., etc.) |
...
| **Labor** | Comissões + CLT + contratos externos | Todos os gastos com pessoal médico |
| **COGS** | Labs externos, insumos, facilities, T.I. | Despesas diretas de funcionamento |
| **Pré-tributação** | Faturamento − Labor − COGS | Resultado antes dos impostos |
| **Lucro Líquido** | Pré-tributação − Impostos estimados | 🟢 Verde = saudável · 🔴 Vermelho = prejuízo |
| **Margem Líquida** | Lucro ÷ Faturamento × 100 | Acima de 20% = excelente · Abaixo de 10% = alerta |

### 5.3 Grupos de Custo

O sistema classifica automaticamente cada categoria da planilha em um grupo:

| Grupo | Ícone | Exemplos de categoria |
|-------|-------|-----------------------|
| Labor | 👥 | Comissões, CLT, horistas, recepção |
| Laboratório Ext. | 🧪 | Laboratório externo, lab |
| Insumos e Med. | 💊 | Insumos, medicamentos |
| Facilities | 🏗️ | Facilities, manutenção predial |
| T.I. | 💻 | T.I., internet, informática |
| Administrativo | 📋 | Administrativo, escritório, estoque |
| Tributação | 🏛️ | Impostos, ICMS, ISS, COFINS, PIS |
| Op. Diverso | 🏷️ | Banho, tosa, marketing, publicidade |
| Outros | 📦 | Categorias não classificadas |

> Use o filtro **"Mês"** no topo para ver dados de um período específico. Sem filtro, todos os meses são somados.
...
| **Semana** | Visão Geral (LaborBI) | Visualiza a produção por semana do mês |
| **Categoria** | CogsBI | Filtra lançamentos por categoria de custo |

Para navegar entre abas, use os botões na barra superior (ex: `Visão Geral`, `Ranking`, `Clínica Médica`).
...
| Margem Líquida | 🟢 Verde | Operação lucrativa |
| Margem Líquida | 🔴 Vermelho | Prejuízo no período analisado |
| Barra de ranking | Azul → Roxo | Proporcional ao maior valor da lista |

---

## 9. Live Preview (Desenvolvimento)

Para visualizar mudanças no `index.html` em tempo real sem precisar reabrir o arquivo manualmente, use o servidor de live preview incluído no projeto.
...
Abra **http://localhost:3000** em uma nova janela do navegador. A cada salvamento do `index.html`, o browser recarrega automaticamente via SSE — sem dependências externas.

> Este recurso é apenas para desenvolvimento. Usuários finais acessam o arquivo diretamente ou via link fornecido pelo administrador.
...
**Por que a aba principal do LaborBI se chama "Visão Geral"?**
A aba foi renomeada para deixar mais claro que ela exibe o resumo consolidado de produção e comissões de toda a equipe veterinária.
